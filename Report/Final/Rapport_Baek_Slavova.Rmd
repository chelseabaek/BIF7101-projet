...
- **Hébergement** : Nous hébergeons notre site pour le rendre accessible aux utilisateurs d’Internet. Notre site Web est hébergé sur [**DigitalOcean**](https://www.digitalocean.com). Nous avons enregistré notre nom de domaine via [**Name.com**](https://www.name.com) et choisi DigitalOcean pour l'hébergement, en profitant du [**GitHub Student Developer Pack**](https://education.github.com/pack), qui nous offrait un nom de domaine gratuit pendant un an et 200 $ de crédits DigitalOcean. (L'environnement d'hébergement offre des ressources de calcul plus limitées que nos ordinateurs, notamment en termes de RAM et de cœurs CPU. Étant donné que certaines analyses phylogénétiques peuvent être intensives, le site Web peut subir des arrêts lors du traitement de grands ensembles de données.)
...
- **MUSCLE :** Par rapport à la version disponible sur T-REX, v3, PhyloDendron intègre une implémentation plus récente de MUSCLE, v5, qui comprend des stratégies d'optimisation améliorées pour l'alignement de séquences multiples [@muscle], notamment une meilleure précision d'alignement.

- **MAFFT :** Nous avons implémenté une version plus récente de MAFFT v7, par rapport à celle disponible sur T-REX, MAFFT v6.864. Cette implémentation mise à jour offre une exécution plus rapide ainsi que des options de stratégies d'alignement telles que des `iterative refinement methods` que nous offrons à l'utilisateur [@mafft].
